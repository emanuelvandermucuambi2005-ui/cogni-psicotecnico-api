...
export function createSession(userId:string,testId:string){
 ...
 const s:Session={id,userId,testId,startedAt:new Date().toISOString(),questionIds:[...test.questionIds],currentIndex:0,score:0,xp:0,correct:0,answered:0,streak:0,finished:false};
...
export function answerSession(sessionId:string,questionId:string,answer:string){
 ...
 s.answered++; if(correct){s.correct++;s.streak++;s.score+=100+s.streak*10;s.xp+=q.xp;} else {s.streak=0;}
 s.currentIndex=Math.min(s.currentIndex+1,s.questionIds.length);
 if(s.currentIndex>=s.questionIds.length)s.finished=true;
 return {correct,correctAnswer:q.answer,explanation:q.explanation,score:s.score,xp:s.xp,streak:s.streak,finished:s.finished};
}
...
export function getProgress(userId:string){
 const ss=listSessions(userId), answered=ss.reduce((a,s)=>a+s.answered,0), correct=ss.reduce((a,s)=>a+s.correct,0), xp=ss.reduce((a,s)=>a+s.xp,0);
 return {userId,xp,answered,correct,accuracy:answered?Math.round(correct/answered*100):0,testsCompleted:ss.filter(s=>s.finished).length,level:Math.max(1,Math.floor(xp/500)+1),streak:Math.max(0,...ss.map(s=>s.streak))};
}
